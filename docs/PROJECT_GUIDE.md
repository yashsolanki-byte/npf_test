...
| **Manual scrape (web UI)** | Same Script_Scraper–style flow in **headless** mode: institute, source, dates, optional advanced filters; saves CSV (or screenshot) to the **user’s OS Downloads folder** (path resolved on the machine running Flask). **Skips** Campaign Detailed View navigation (post-login stays on current page). |
...
flowchart TB
  ...
  subgraph engines [Python engines - scrapers/]
    ...
    SSH[script_scraper - run_headless]
...
| Manual scrape | `routes/manual_scrape.py` | `GET /api/manual-scrape/urls`, `POST /filters`, `/subfilter-options`, `/run`, `GET /status`, `/download` |
...
| `POST /api/stop/manual` | Manual scrape stops after the **current page** (CSV may be skipped if stopped early). |
...
3. **`POST /api/manual-scrape/filters`** / **`subfilter-options`**: spawn **`ScraperApp(root=None)`** with `headless=True` (server-safe); uses **`skip_campaign_detailed_view`** so Detailed View is **not** used for web manual paths.
4. For web manual mode, Playwright Chromium launch uses `--no-proxy-server` (manual scraper proxy = none).
5. **`POST /api/manual-scrape/run`**: thread calls **`run_headless(params, status_callback)`** → **`async_scraper` → `run_scraping`**.
6. Output: CSV and screenshots go to the **Downloads** folder (`get_user_downloads_dir()` in `path_utils`: Windows registry path when available, else `~/Downloads`). Filenames like `<safe_institute>_<dd-mm-yy>.csv` (or `_HHMMSS` suffix if the name already exists).
7. If **Primary Leads** count **> `NPF_MANUAL_SCRAPE_MAX_LEADS`** (default 1 lakh), raises **`ManualScrapeLeadsLimitExceeded`** → surfaced as **`manual_scrape_status["error"]`**.
8. Poll **`GET /api/manual-scrape/status`**; download via **`/api/manual-scrape/download?file=<basename>`** (Downloads only). Legacy **`?path=`** under `DATA_Scraped` remains for old files.
