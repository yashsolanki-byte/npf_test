...
# Project root (must be before logging — paths define logs/script/)
...
# Path helpers – project root (this script lives in npf-scraper-webapp)
_WEBAPP_DIR = _ROOT
...
# Filter cache file (per institute+source)
FILTER_CACHE_FILE = paths.FILTER_CACHE_JSON
# Institutes list for autocomplete
INSTITUTES_JSON = paths.INSTITUTES_JSON
# Publisher URLs for autocomplete
        ...
        try:
            ...
            if params.get("screenshot_mode"):
                ...
                if not has_no_records and not has_primary_leads:
                    ...
                    self._schedule( lambda: messagebox.showerror("Screenshot Aborted", "Screenshot can only be taken when the page shows either:\n\n• No Record Found\n• Total ... Primary Leads\n\nPlease wait for the page to load fully and try again."))
                    await context.close()
                    await browser.close()
                    self.scraping_complete()
                    return
...
def run_headless(params, status_callback=None):
    """
    Run the scraper headless (no GUI). Used by the webapp.
    params: dict with login_url, email, password, institute, source, from_date, to_date,
            instance, rows_per_page, order, filename, output_dir, advanced_filter_ids, subfilter_options.
    status_callback: optional callable(message) for status updates.
    Returns: output_path (str) of saved CSV or screenshot, or None on failure.
    """
    _stub_messagebox()
    app = ScraperApp(root=None)
    app.status_callback = status_callback
    try:
        asyncio.run(app.async_scraper(params))
